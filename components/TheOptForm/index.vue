...
<template>
  <section class="opt">
    <div class="opt__container container">
      <img class="hidden md:block" src="/assets/images/black-friday.png" alt="friday">
      <div class="flex flex-col items-start relative pb-7">
        <h3>Хотите получить выгодное коммерческое предложение?</h3>
        <p class="text-gray-400 text-[14px] font-semibold mb-4">Заполните форму и мы сразу с Вами свяжемся.</p>
        <form @submit.prevent="sendForm" class="flex flex-col items-stretch sm:items-start w-full">
          <div class="w-full flex flex-wrap md:gap-3">
            <div class="md:flex w-full md:w-2/5 md:items-center justify-center mb-4">
              <div class="w-full">
                <input type="hidden" name="form_id" value="37"/>
                <input
                    class="bg-gray-100 appearance-none border-2 border-gray-100 rounded-3xl w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-yellow-300"
                    id="inline-full-name"
        ...
        <div v-if="errorMessageCaptcha" class="error text-[14px] transition-all animate-bounce absolute bottom-0">Ошибка: Введите капчу!</div>
      </div>
    </div>
  </section>
</template>
