...
<template>
  <div class="modal__container">
    <h3 class="font-semibold text-[20px] sm:text-2xl text-black mb-5 text-center ">{{title}}</h3>
    <form @submit.prevent="sendForm" class="flex flex-col w-full">
      ...
      <UCheckbox color="yellow" class="my-3" v-model="checkbox">
        <template #label>
          <span class="text-gray-600">Я даю согласие на обработку моих персональных данных ООО «» (ИНН ) в целях обработки заявки и обратной связи. <router-link class="text-yellow-500" target="_blank" to="/assets/policyO.pdf">Политика конфиденциальности</router-link> </span>
        </template>
      </UCheckbox>
