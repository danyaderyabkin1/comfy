...
const customCanonical = computed(()=> {
  return product?.value?.fields?.find((el) => el.field_title === 'canonical')
})

const config = useRuntimeConfig()
const canonicalUrl = `${config.public.siteUrl}${customCanonical?.value?.field_value ? customCanonical?.value?.field_value : route.path}`
    ...
    <section class="product">
      <div class="container product__container">
        <UBreadcrumb
            class="mb-8"
            divider="/"
            :links="[{ label: 'Главная', to: '/' }, {label: 'Номера под ключ'}, { label: cleanText(product?.preview_content) }]"
        />
        <div class="product__wrap">
          ...
          <div class="flex-1">
            ...
            <strong v-if="product?.sku.length">Цена: <span v-for="sku in product?.sku" :key="sku.id">{{ sku.price }} ₽</span></strong>
